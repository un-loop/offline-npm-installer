...
const libs: string[] = fse.readFileSync('packages')
                .toString()
                .split("\n")
...
function startVerdaccio() {
    ...
    const verdaccio = childProcess.exec('node node_modules\\verdaccio\\build\\lib\\cli');
...
function installPackages(verdaccio: childProcess.ChildProcess) {
    ...
    function installPackage() {
        console.log(`Installing '${libs[packageNum]}' (${packageNum + 1}/${libs.length})`);

        let install = childProcess.exec(`npm install ${libs[packageNum]} --registry http://localhost:4873/ --no-save`);

        install.stdout.on('data', (data: string) => console.log(`[npm stdout]: ${data.trim().replace("\n", " ")}`));
        install.stderr.on('data', (data: string) => console.error(`[npm stderr]: ${data.trim().replace("\n", " ")}`));
...
function stopVerdaccio(verdaccio: childProcess.ChildProcess) {
    ...
    childProcess.spawn('taskkill', ['/pid', verdaccio.pid.toString(), '/f', '/t']).on('exit', packageCache);
}
